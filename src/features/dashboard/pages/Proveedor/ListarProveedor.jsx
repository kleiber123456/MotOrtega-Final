...
const ListarProveedor = () => {
    ...
    const fetchProveedores = async () => {
        try {
            ...
            const res = await axios.get("https://api-final-8rw7.onrender.com/api/proveedores", {
                headers: {
                    "Authorization": `${token}`,
    ...
    const eliminarProveedor = async (id) => { // 'id' aquí es el _id del proveedor
        if (!id) {
        ...
        try {
            const res = await axios.delete(`https://api-final-8rw7.onrender.com/api/proveedores/${id}`, {
                headers: {
                    "Authorization": `${token}`,
                    "Content-Type": "application/json"
                }
            ...
            if (res.status === 200 || res.status === 204) { // 200 OK o 204 No Content son respuestas válidas para DELETE
    ...
    const cambiarEstado = async (id) => { // 'id' aquí es el _id del proveedor
        try {
            if (!token) {
                Swal.fire("Error", "No autorizado: Token no encontrado.", "error");
                return;
            ...
            Swal.fire("Éxito", "Estado del proveedor cambiado correctamente", "success");
        } catch (err) {
    ...
    const proveedoresFiltrados = Array.isArray(proveedores) ? proveedores.filter((prov) => {
        ...
        return (
            ...
            String(prov.telefono || '').toLowerCase().includes(textoBusqueda) ||
    ...
    const proveedoresPaginados = proveedoresFiltrados.slice(indexPrimero, indexUltimo);
